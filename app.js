...
(function () {
  ...
  function updateLocalStreakOnAnalyze() {
    ...
    if (last === today) return loadStreak(); // already counted today
    const diffDays =
  ...
  async function refreshUser() {
    ...
    if (currentUser) {
      ...
      showLeaderboard();
      computeStreakFromSupabase(); // upgrade streak display if signed-in
    } else {
  ...
  async function startRec() {
    ...
    if (!currentUser && localSessions >= 1) {
      alert("Sign in to save progress and show up in the leaderboard.");
    }
  ...
  async function analyze() {
    ...
    try {
      ...
      if (supabase && currentUser) {
        ...
        showLeaderboard();
        // recompute streak from server to be accurate across devices
  ...
  // ===== Streak from Supabase (optional, more accurate) =====
  async function computeStreakFromSupabase() {
    ...
    try {
      ...
      const days = new Set(
        data.map((r) => String(r.created_at).slice(0, 10))
      );
      // Walk backwards from today while days contain the key
      let s = 0;
      ...
      streakEl.textContent = String(s);
      // also mirror locally so guest view persists
      saveStreak(s);
      saveLastDay(todayKey());
    } catch {
      // ignore
    }
  }
